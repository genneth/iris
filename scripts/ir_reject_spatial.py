...
def main() -> None:
    ...
    try:
        for i, frame in enumerate(cam):
            if i < N:                                   # phase 1: IR off
                off_sum += y_image(frame); off_n += 1
                if i == N - 1:
                    ir_thread = threading.Thread(target=ir_worker, args=(stop, ir_count))
                    ir_thread.start()
            elif i < 2 * N + 4:                          # phase 2: IR on (skip 4 spin-up)
                if i >= N + 4:
                    on_sum += y_image(frame); on_n += 1
            else:
                break
    finally:
        if ir_thread is not None:
            stop.set(); ir_thread.join()
        set_control(cam, 0x009A0901, 3); set_control(cam, 0x00980913, 64)
        cam.close()
    ...
    print(f"max-diff pixel at (row={r}, col={c})  {'CENTER (emitter-like)' if central else 'edge (motion-like)'}")
    ...
    print(f"wrote ir_reject_diff.png (stretched: full white = +5 luma) and ir_reject_on.png to {OUT}/")
