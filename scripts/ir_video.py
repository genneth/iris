...
def main() -> None:
    ...
    print(f"emitter-ON brightness: mean={np.mean([f.mean() for f in on]):.1f}, "
    ...
    onv[0].save(f"{OUT}/ir_video.gif", save_all=True, append_images=onv[1:],
                duration=133, loop=0)

    allv = [stretch(f, hi) for f in raw]
    allv[0].save(f"{OUT}/ir_strobe.gif", save_all=True, append_images=allv[1:],
                 duration=67, loop=0)
    print(f"wrote ir_nightvision.png, ir_video.gif ({len(onv)} frames), "
