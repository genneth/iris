...
REPORT_DESC = (
    UP_SENSOR + ALS_TYPE + [0xA1, 0x00]          # Collection (Physical), ALS -- single (majbthrd style)
    + [0x85, 0x01]                               # REPORT_ID(1) -- Linux hid-sensor-hub requires it
    + PROP_REPORT_INTERVAL                       # feature: report interval (u32)
    + [0x15, 0x00] + [0x27, 0xFF, 0xFF, 0xFF, 0xFF]
    + [0x75, 0x20, 0x95, 0x01, 0x55, 0x00, 0xB1, 0x02]
    + REP_STATE + [0x15, 0x00, 0x25, 0x05, 0x75, 0x08, 0x95, 0x01] + COLL_LOG
    + sum(REP_STATE_SEL, []) + [0xB1, 0x00] + END   # feature: reporting state (NAry)
    + PWR_STATE + [0x15, 0x00, 0x25, 0x05, 0x75, 0x08, 0x95, 0x01] + COLL_LOG
    + sum(PWR_STATE_SEL, []) + [0xB1, 0x00] + END    # feature: power state (NAry, mandatory)
    + STATE + [0x15, 0x00, 0x25, 0x06, 0x75, 0x08, 0x95, 0x01] + COLL_LOG
    + sum(STATE_SEL, []) + [0x81, 0x00] + END     # input: sensor state (array)
    + EVENT + [0x15, 0x00, 0x25, 0x10, 0x75, 0x08, 0x95, 0x01] + COLL_LOG
    + sum(EVENT_SEL, []) + [0x81, 0x00] + END      # input: sensor event (array)
...
def validate():
    ...
    found = [f"{f.usage_name}" for r in rd.input_reports.values() for f in r
...
def run():
    ...
    while time.monotonic() - t0 < dur:
        ...
        now = time.monotonic()
        if now >= next_send:
            dev.send_lux(lux); next_send = now + 1.0; lux = min(lux + 100, 4000)
        if not reported and now - t0 > 3:
            ...
            for ln in subprocess.run(["dmesg"], capture_output=True, text=True).stdout.splitlines()[-120:]:
